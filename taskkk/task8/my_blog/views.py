...
def search_post(request):
    if request.method == 'POST':
        t_data = request.POST["name"]
        post_list = Blog_Post.objects.filter(name__icontains = t_data)
        context = {"post_list":post_list
                    }
        return render(request,'blog/search_result.html',context)
    return render(request,'blog/search_from.html')




def search_categori(request):
    if request.method == 'POST':
        t_data = request.POST["name"]
        post_list = Blog_Post.objects.filter(catagory__name__icontains = t_data)
        context = {"post_list":post_list
                    }
        return render(request,'blog/categorisrcresult.html',context)
    return render(request,'blog/catagorysrc.html')  
...
def search_both(request):
    if request.method == 'POST':
        t_data = request.POST["name"]
        post_list = Blog_Post.objects.filter(Q(catagory__name__icontains = t_data)|Q(name__icontains = t_data))
        context = {"post_list":post_list
                    }
        return render(request,'blog/bothresult.html',context)
    return render(request,'blog/bothsrc.html')  